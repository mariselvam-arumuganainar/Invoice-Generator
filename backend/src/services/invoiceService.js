...
const getNextInvoiceNumber = async () => {
  // PostgreSQL-compatible query to get the next invoice number
  ...
  const nextNum = (rows[0].maxnum || 0) + 1; // Note: maxnum is lowercase
...
const createInvoice = async (invoiceData) => {
  ...
  try {
    ...
    // Loop through items and insert them one by one (safe for transactions)
